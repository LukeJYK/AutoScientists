...
ok.md — Orchestrator Runbook (base program)
...
### Reading code blocks in this file

This file mixes two different kinds of code block. Confusing them is the most common way an orchestrator run silently fails:

1. **Real code** — plain Python/bash meant to be executed for real (Step 1's bootstrap, `requests.get(...)` calls, file reads/writes). Actually run it: write a script and execute it, or make the real HTTP/file call yourself.
2. **Tool-call pseudocode** — anything shaped like `Agent(...)` or `Task(...)`, marked `# TOOL CALL` above it. This is not a Python function you can import or run with `python3` — it is literal shorthand for "call your own Task/Agent tool right now, with these exact arguments." When you see it, invoke that tool directly. Do not write it to a file, do not try to execute it as a script, and do not skip it.

If you ever catch yourself reasoning "I can't run this as Python because it uses the Claude Code agent interface" — stop, that thought is the bug. It means you're looking at tool-call pseudocode (kind 2), and the fix is to call the tool, not to give up and describe what you imagine would happen instead.
...
**NEVER FABRICATE A RESULT. NEVER NARRATE INSTEAD OF ACTING.**
If a step can't be completed — a tool call fails, an API returns an error, a file is missing — stop and report the blocker explicitly. Never substitute a plausible-sounding account of "what would happen" or "what the result would be" for actually doing it. This failure mode is documented for weaker models acting as analysts (see the model note in Step 3) but applies to every role, including the orchestrator itself.
...
for agent_name in non_admin_agents:
    # TOOL CALL — invoke your Task/Agent tool now with these args. Not a Python function.
    Agent(
